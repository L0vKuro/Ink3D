...
export async function POST(req) {
  ...
  const { success } = await ratelimit.limit(`forgotpw_${ip}`);
  if (!success) {
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="background:#050505;padding:40px;font-family:monospace;color:#fff;max-width:600px;">
        ...
        <a href="https://ink3dshop.com/dashboard/reset?token=${token}" style="display:block;background:#ae1fe3;color:#fff;text-align:center;padding:16px;font-weight:900;font-size:12px;letter-spacing:3px;text-decoration:none;margin-bottom:24px;">
          RESET PASSWORD →
        </a>
