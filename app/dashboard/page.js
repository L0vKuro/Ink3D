        ...
        <div className="w-full max-w-sm">
          ...
          {!showForgot ? (
            <>
              ...
              <button onClick={() => setShowForgot(true)} className="w-full mt-4 font-mono-custom text-[9px] text-white/20 hover:text-white/50 transition-colors tracking-widest text-center">
                FORGOT PASSWORD?
              </button>
            </>
          ) : (
            <>
              {forgotStatus === "sent" ? (
                <div className="text-center">
                  ...
                  <div className="font-mono-custom text-[9px] text-green-400 tracking-widest mb-4">// CHECK YOUR EMAIL</div>
                  <p className="font-mono-custom text-white/30 text-sm mb-6">If that email is in our system, a reset link is on its way. Check your inbox.</p>
                  <button onClick={() => { setShowForgot(false); setForgotStatus(null); setForgotEmail(""); }}
                    className="font-mono-custom text-[9px] text-white/30 hover:text-white transition-colors tracking-widest">
                    ← BACK TO LOGIN
                  </button>
                </div>
              ) : (
                <>
                  <form onSubmit={handleForgot} className="space-y-4">
                    <div>
                      ...
                      <input type="email" value={forgotEmail} onChange={e => setForgotEmail(e.target.value)} placeholder="your@email.com" className={inputClass} required />
                    </div>
                    <button type="submit" disabled={forgotLoading}
                      className="w-full py-4 font-black text-xs tracking-[0.25em] font-mono-custom transition-all duration-200 glow-btn"
                      style={{background: forgotLoading ? '#ae1fe380' : '#ae1fe3', color: '#fff'}}
                      onMouseEnter={e => { if (!forgotLoading) e.currentTarget.style.background='#c040ff'; }}
                      onMouseLeave={e => { if (!forgotLoading) e.currentTarget.style.background='#ae1fe3'; }}>
                      {forgotLoading ? '[ SENDING... ]' : '[ SEND RESET LINK ]'}
                    </button>
                  </form>
                  <button onClick={() => setShowForgot(false)} className="w-full mt-4 font-mono-custom text-[9px] text-white/20 hover:text-white/50 transition-colors tracking-widest text-center">
                    ← BACK TO LOGIN
                  </button>
                </>
              )}
